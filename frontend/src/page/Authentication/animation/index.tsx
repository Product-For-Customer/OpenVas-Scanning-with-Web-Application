...
// ─── Theme ───────────────────────────────────────────────────────
// dark  → electric cyan/purple on near-black
// light → sky-blue/indigo on light blue-gray
const T = {
  dark: {
    bg:          "#050a0e",
    primary:     "#00e5ff",
    ...
    globe:       "#0a1628",
    lc:          "#0a4080",
    emI: 0.90,   oLon: 0.32,  oLat: 0.20,  oEq: 0.65,
    vigRgb:      "5,10,14",
    vigStr:      0.82,
    panelBg:     "rgba(5,10,14,0.88)",
  ...
  light: {
    ...
    globe:       "#bfdbfe",
    lc:          "#60a5fa",
    emI: 0.45,   oLon: 0.25,  oLat: 0.18,  oEq: 0.45,
    vigRgb:      "220,234,248",
    vigStr:      0.65,
    panelBg:     "rgba(238,244,252,0.92)",
    panelBorder: "rgba(2,132,199,0.28)",
    matFade:     "rgba(238,244,252,0.07)",
    matBright:   "#0284c7",
    matOpacity:  0.045,
  },
};
...
type Theme = {
  bg: string; primary: string; secondary: string; safe: string;
  globe: string; lc: string;
  emI: number; oLon: number; oLat: number; oEq: number;
  vigRgb: string; vigStr: number;
  panelBg: string; panelBorder: string;
  matFade: string; matBright: string; matOpacity: number;
};
...
const Globe: React.FC<{ active: Set<number>; phase: Phase; theme: Theme; isDark: boolean }> = ({
  ...
  return (
    <group ref={gRef}>
      <mesh>
        ...
        <meshStandardMaterial color={theme.globe} transparent opacity={isDark ? 0.92 : 0.55} />
      </mesh>
      {/* Outer glow rim */}
      <mesh>
        <sphereGeometry args={[2.2, 32, 32]} />
        <meshStandardMaterial ref={rimMat} color={theme.primary} emissive={theme.primary}
          emissiveIntensity={isDark ? 0.12 : 0.06} transparent opacity={isDark ? 0.04 : 0.02} side={THREE.BackSide} />
      </mesh>
      {/* Longitude lines */}
      {LONS.map((a, i) => (
      ...
      {ATTACKS.map((a, i) => active.has(i) && (
        <React.Fragment key={i}>
          ...
          <GeoNode pos={a.dst} col={isDark ? "#ffffff" : "#1e3a5f"} />
      ...
      <group ref={gRef}>
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshStandardMaterial ref={mat} color={theme.primary} emissive={theme.primary}
            emissiveIntensity={2.2} transparent opacity={0.88} />
        </mesh>
        ...
        <mesh position={[2.06, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
          ...
          <meshStandardMaterial color={theme.primary} emissive={theme.primary}
            emissiveIntensity={2} transparent opacity={0.3} />
        </mesh>
      </group>
    </group>
  );
...
// ─────────────────────────────────────────────────────────────────
// HTML ── Matrix rain — adapts fade color per theme
// ─────────────────────────────────────────────────────────────────
const MatrixRain: React.FC<{ color: string; fade: string; bright: string; opacity: number }> = ({
  ...
  return <canvas ref={ref} style={{ position: "absolute", inset: 0, opacity, zIndex: 2, pointerEvents: "none" }} />;
};
...
const ScanBeam: React.FC<{ color: string }> = ({ color }) => (
  <motion.div
    ...
    style={{
      position: "absolute", left: 0, right: 0, height: 1.5, zIndex: 7, pointerEvents: "none",
...
const Animation: React.FC<Props> = ({ onFinished, duration = 5000 }) => {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setPhase("complete"), Math.max(TOTAL - 800, 3600)),
      setTimeout(() => setPhase("exit"),     Math.max(TOTAL - 300, 4150)),
    ...
    let raf = 0;
    const frame = () => {
      setProgress(Math.min(((performance.now() - start) / TOTAL) * 100, 100));
      setTick(t => t + 1);
  ...
  const ambientIntensity = isDark ? 0.04 : 0.25;
  const pointInt1        = isDark ? 10   : 6;
  const pointInt2        = isDark ? 5    : 3;
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 5 }}>
        <Canvas
          camera={{ position: [0, 1.4, 8.0], fov: 44 }}
          gl={{ antialias: true, alpha: false }}
          dpr={[1, 1.5]}
        >
          <CanvasBg bg={theme.bg} />
          <ambientLight intensity={ambientIntensity} />
          <pointLight position={[5, 6, 6]}   color="#ffffff"        intensity={pointInt1} distance={24} />
          <pointLight position={[-4,-4,-5]}  color={theme.secondary} intensity={pointInt2} distance={20} />
          {!isDark && <pointLight position={[0, 0, 6]} color={theme.primary} intensity={2} distance={16} />}
      ...
      <div style={{
        ...
        background: `radial-gradient(ellipse 75% 75% at 50% 52%, transparent 30%, rgba(${theme.vigRgb},${theme.vigStr}) 100%)`,
      }} />
      ...
      {active.size > 0 && (
        <motion.div
          initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}
          style={{
            position: "absolute", top: 50, right: 22, zIndex: 20, pointerEvents: "none",
      ...
      {phase !== "init" && (
        <motion.div
          initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.7 }}
          style={{
            position: "absolute", top: 50, left: 22, zIndex: 20, pointerEvents: "none",
      ...
      >
        <div style={{
          width: "100%", position: "relative", height: 6,
