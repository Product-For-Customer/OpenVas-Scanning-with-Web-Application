...
const Index: React.FC<Props> = ({ onFinished, duration = 1000 }) => {
  ...
  const streamDots = useMemo(() => Array.from({ length: 7 }, (_, i) => i), []);
  const pulseRings = useMemo(() => Array.from({ length: 5 }, (_, i) => i), []);
  const radarSweeps = useMemo(() => Array.from({ length: 4 }, (_, i) => i), []);
  const dataPackets = useMemo(() => Array.from({ length: 12 }, (_, i) => i), []);
  const backgroundDots = useMemo(() => Array.from({ length: 40 }, (_, i) => i), []);
      ...
      <style>{`
        * {
        ...
        .scene {
          ...
          justify-content: center;
          z-index: 20;
          /* isolate this subtree's layout/paint from the rest of the page so the
             many looping child animations never trigger reflow outside .scene */
        ...
        @keyframes scanPass {
          ...
          100% {
            transform: skewY(-10deg) translateY(1422%);
            opacity: 0;
          }
        }
