...
// Shared input styles — matched to the ThreatConfig "New Credential" modal so
// the digest editor feels like the rest of the app's create dialogs.
const inputCls =
...
// Digest runs in the app's configured timezone; the picker was removed from the
// modal, so new schedules default to this and existing ones keep their value.
...
// ── Create/Edit modal — styled like the "New Credential" dialog ──────────────
...
const DigestModal: React.FC<{
  ...
  return createPortal(
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
      ...
      <div
        className="relative z-10 w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#12101f]"
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/8">
          <div className="flex items-center gap-2.5">
            <span className="flex h-8 w-8 items-center justify-center rounded-xl text-white" style={{ background: accentGrad }}>
              {initial ? <FiEdit2 className="text-[14px]" /> : <FiMail className="text-[14px]" />}
            </span>
            <div>
              <p className="text-[9.5px] font-bold uppercase tracking-widest" style={{ color: currentColor }}>AUTO-REPORT DIGEST</p>
              <h3 className="text-[14px] font-bold text-slate-800 dark:text-white/90">{initial ? "Edit Report Digest" : "New Report Digest"}</h3>
            </div>
          </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
            ...
            <div className="flex gap-2">
              {(["email", "line"] as DigestChannel[]).map((ch) => (
                <button key={ch} type="button" onClick={() => setChannel(ch)}
                  className={`flex flex-1 items-center justify-center gap-2 rounded-lg border py-2 text-[12.5px] font-medium capitalize transition ${channel === ch ? "border-blue-400 bg-blue-50 text-blue-600 dark:border-blue-500/40 dark:bg-blue-500/10" : "border-slate-200 text-slate-500 dark:border-white/10 dark:text-white/55"}`}>
          ...
          {channel === "email" ? (
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-400 dark:text-white/30">Comma-separated. Uses the SMTP account from Service settings.</p>
            </div>
          ) : (
            <div>
              <label className={labelCls}>LINE targets</label>
              {notifications.length === 0 ? (
                <p className="text-[12px] text-slate-400 dark:text-white/30">No LINE notifications configured — add one on the LINE page first.</p>
              ) : (
                <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-slate-200 p-2 dark:border-white/10">
                  {notifications.map((n) => (
                    <label key={n.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-[12.5px] hover:bg-slate-50 dark:hover:bg-white/5">
                      <input type="checkbox" checked={lineIds.has(n.id)} onChange={() => toggleLine(n.id)} className="accent-blue-500" />
        ...
        <div className="flex shrink-0 gap-2 border-t border-slate-100 px-5 py-4 dark:border-white/8">
          ...
          <button type="button" onClick={() => void save()} disabled={saving || !name.trim()} style={{ background: accentGrad }}
    ...
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#12101f]">
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div className="flex gap-2">
            <button onClick={onCancel} disabled={loading} className="flex-1 rounded-xl border border-slate-200 py-2 text-[12.5px] font-semibold text-slate-600 dark:border-white/10 dark:text-white/60">Cancel</button>
            <button onClick={onConfirm} disabled={loading} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-500 py-2 text-[12.5px] font-semibold text-white disabled:opacity-60">
...
// ── Section (embeds into the Service page) ──────────────────────────────────

const ReportDigestSection: React.FC = () => {
  const { currentColor } = useStateContext();
  const { t } = useLanguage();
  const { can } = useAuth();
  ...
  // Hidden entirely for roles without dashboard view (the digest APIs are gated
  // under the "dashboard" permission category on the backend).
  if (!canView) return null;

  return (
    <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white dark:border-white/8 dark:bg-[#0d0b1a]/60">

      {/* Section header — matches the Service page's other section cards */}
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-3.5 dark:border-white/8">
        <div className="flex items-center gap-2.5">
          <FiMail className="text-[14px] text-slate-400 dark:text-white/35" />
          <div>
            <h2 className="text-[13px] font-bold text-slate-800 dark:text-white/90">{t("nav.reportDigest")}</h2>
        ...
        {canManage && (
          <button onClick={() => setModal({ mode: "create" })} style={{ background: accentGrad }}
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-[12px] font-semibold text-white transition hover:opacity-90">
            <FiPlus size={14} /> New
          </button>
        )}
      </div>
      ...
      <div className="p-5">
        {loading ? (
          <div className="space-y-2.5">
            {[1, 2].map((i) => (
              <div key={i} className="h-20 animate-pulse rounded-xl border border-slate-100 bg-slate-50 dark:border-white/8 dark:bg-white/4" />
            ))}
          </div>
        ) : digests.length === 0 ? (
          <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-slate-200 py-12 text-center dark:border-white/10">
            <FiClock className="text-2xl text-slate-300 dark:text-white/25" />
            <p className="text-[13px] font-semibold text-slate-600 dark:text-white/60">No scheduled digests yet.</p>
            {canManage && <p className="text-[11px] text-slate-400 dark:text-white/35">Click “New” to schedule your first automated report.</p>}
          </div>
        ) : (
          <div className="space-y-3">
            {digests.map((d) => (
              <div key={d.id} className="rounded-xl border border-slate-200/70 bg-slate-50/60 p-4 dark:border-white/8 dark:bg-white/3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-[13px] font-semibold text-slate-800 dark:text-white/90">{d.name}</span>
                      ...
                      {!d.enabled && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-500 dark:bg-white/10 dark:text-white/45">paused</span>}
                      {d.last_status === "ok" && <span className="inline-flex items-center gap-1 text-[11px] text-emerald-600 dark:text-emerald-400"><FiCheckCircle size={10} /> last ok</span>}
                    ...
                    <p className="mt-1 flex items-center gap-1.5 text-[12px] text-slate-500 dark:text-white/45">
                      <FiCalendar size={12} /> {scheduleSummary(d)} · {d.timezone}
                    </p>
                    <p className="mt-0.5 text-[11.5px] text-slate-400 dark:text-white/35">
                  ...
                  {canManage && (
                    <div className="flex shrink-0 items-center gap-1.5">
                      <button onClick={() => toggleEnabled(d)} title={d.enabled ? "Pause" : "Resume"}
                        className="rounded-lg border border-slate-200 px-2 py-1.5 text-[11px] font-medium text-slate-500 hover:bg-white dark:border-white/10 dark:text-white/50 dark:hover:bg-white/5">
                        {d.enabled ? "Pause" : "Resume"}
                      </button>
                      <button onClick={() => void runNow(d)} disabled={busyId === d.id} title="Send now"
                        className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-emerald-600 hover:bg-emerald-50 disabled:opacity-40 dark:border-white/10 dark:hover:bg-emerald-500/10">
                      ...
                      <button onClick={() => setModal({ mode: "edit", digest: d })} title="Edit"
                        className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-white dark:border-white/10 dark:text-white/50 dark:hover:bg-white/5">
                        <FiEdit2 size={13} />
                      </button>
                      <button onClick={() => setDeleteTarget(d)} title="Delete"
                        className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-red-50 hover:text-red-500 dark:border-white/10 dark:text-white/50">
                        <FiTrash2 size={13} />
                      </button>
                    </div>
                  )}
...
export default ReportDigestSection;
